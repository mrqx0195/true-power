...
public final class TrickHandler {
    ...
    public static void trickDown(InputCommandEvent event) {
        ...
        if (blade.isEmpty()
            ...
            || (data != null && data.getAvoidTrick() > 0)) {
            return;
        }
    ...
    public static void onTick(TickEvent.PlayerTickEvent event) {
        ...
        ITruePowerData data = ITruePowerData.get(event.player);
        ...
        if (count <= 0 && event.player instanceof ServerPlayer serverPlayer) {
            serverPlayer.hasChangedDimension();
            if (data.isTrickDowning()) {
                if (serverPlayer.getCapability(ItemSlashBlade.INPUT_STATE)
                    .map(s -> s.getCommands(serverPlayer).contains(InputCommand.SPRINT)).orElse(false)) {
                    if (InputStream.getOrCreateInputStream(serverPlayer)
                        ...
                        ItemStack blade = serverPlayer.getMainHandItem();
                        doTrickDown(serverPlayer, data, blade.getCapability(ItemSlashBlade.BLADESTATE).orElse(new SlashBladeState(blade)));
                    }
                } else {
